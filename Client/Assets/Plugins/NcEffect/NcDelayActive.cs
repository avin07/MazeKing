...
/*
	...
	{
// 		Debuger.Log("IsAddedInvoke " + name);
...
// 		Debuger.Log("Awake" + m_fDelayTime + gameObject.name);
		if (m_fDelayTime != 0)
		{
	...
	{
		return;	// not use
// 		Debuger.Log("InitNcDelayActive " + m_fStartedTime + gameObject.name);
...
//  			Debuger.Log("InitNcDelayActive " + fInvokeTime + gameObject.name);
...
// 				Debuger.Log(m_fParentDelayTime);
			}
		}
		return m_fParentDelayTime;
	}
	...
	{
// 		Debuger.Log("CancelDelayActive " + name);
		m_fStartedTime = 0;
	...
	void OnStartActive()
	{
//   		Debuger.Log("OnStartActive" + m_fStartedTime + gameObject.name);
		if (0 < m_fStartedTime)
			return;
