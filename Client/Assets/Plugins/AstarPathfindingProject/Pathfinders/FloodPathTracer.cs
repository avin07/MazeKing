...
namespace Pathfinding
{
	...
	public class FloodPathConstraint : NNConstraint {
		...
		public FloodPathConstraint (FloodPath path) {
			if (path == null) { Debuger.LogWarning ("FloodPathConstraint should not be used with a NULL path"); }
			this.path = path;
		}
	...
	public class FloodPathTracer : ABPath {
		...
		public void Trace (GraphNode from) {
			...
			while (c != null) {
				...
				count++;
				if (count > 1024) {
					Debuger.LogWarning ("Inifinity loop? >1024 node path. Remove this message if you really have that long paths (FloodPathTracer.cs, Trace function)");
					break;
				}
			}
		}
