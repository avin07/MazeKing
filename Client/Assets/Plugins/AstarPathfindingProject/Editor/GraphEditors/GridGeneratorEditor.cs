	...
	public class GridGraphEditor : GraphEditor {
		...
		static void SaveReferenceTexture (GridGraph graph) {
			...
			if (graph.nodes.Length != graph.width * graph.depth) {
				Debuger.LogError ("Couldn't create reference image since width*depth != nodes.Length");
				return;
			}

			if (graph.nodes.Length == 0) {
				Debuger.LogError ("Couldn't create reference image since the graph is too small (0*0)");
				return;
			}
