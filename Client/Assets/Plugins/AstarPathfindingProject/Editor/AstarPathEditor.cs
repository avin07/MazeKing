...
#region Styles
	...
	static void DisableJs () {
		...
		if (System.IO.Directory.Exists (Application.dataPath+"/Plugins/AstarPathfindingProject")) {
			...
			if (error != "") {
				Debuger.LogError ("Couldn't disable Js - "+error);
			} else {
				try {
			...
			Debuger.LogWarning ("Could not disable JS - Could not find directory '"+Application.dataPath+"/Plugins/AstarPathfindingProject'");
		}
		...
		if (System.IO.Directory.Exists (Application.dataPath+"/AstarPathfindingEditor/Editor")) {
			...
			Debuger.LogWarning ("Could not disable JS - Could not find directory '"+Application.dataPath+"/AstarPathfindingEditor/Editor'");
		}
	...
	static bool LocateEditorAssets () {
		...
		if (!System.IO.File.Exists (projectPath + editorAssets + "/AstarEditorSkinLight.guiskin") && !System.IO.File.Exists (projectPath + editorAssets + "/AstarEditorSkin.guiskin")) {
			...
			if (!found) {
				Debuger.LogWarning ("Could not locate editor assets folder. Make sure you have imported the package correctly.\nA* Pathfinding Project");
				return false;
			}
		}
		return true;
	...
	public static bool LoadStyles () {
		...
		if (astarSkin != null) {
			...
			Debuger.LogWarning ("Could not load editor skin at '" + skinPath + "'");
			return false;
		}
		...
		if (topFadeArea.Show () ) {
			...
			if (fadeArea.Show ()) {
				...
				if (nodenull) {
					...
					Debuger.LogError ("Some nodes in the graph are null. Please report this error.");
				}
	...
	#if ASTARDEBUG
					watch.Stop ();
					Debuger.Log ("Saved to file, process took "+(watch.ElapsedTicks*0.0001).ToString ("0.00")+" ms to complete");
	#endif
				}
			}
			...
			if (GUILayout.Button ("Load from file")) {
				...
				if (path != "") {
					...
					try {
						...
						Debuger.LogError ("Could not load from file at '"+path+"'\n"+e);
						bytes = null;
					}
	...
	public static void EditTags () {
		...
		if (astar != null) {
			...
			Debuger.LogWarning ("No AstarPath component in the scene");
		}
	}
		...
		if (forceRebuild || graphEditors == null || script.graphs == null || script.graphs.Length != graphEditors.Length) {
			...
			for (int i=0;i< script.graphs.Length;i++) {
				...
				if (graph.guid == new Pathfinding.Util.Guid ()) {
					Debuger.LogWarning ("Zeroed guid detected, creating new randomized guid");
	...
	GraphEditor CreateGraphEditor (string graphType) {
		...
		Debuger.LogError ("Couldn't find an editor for the graph type '"+graphType+"' There are "+graphEditorTypes.Count+" available graph editors");
	...
	public byte[] SerializeGraphs (Pathfinding.Serialization.SerializeSettings settings, out uint checksum) {
		...
		AstarPath.active.AddWorkItem (new AstarPath.AstarWorkItem (force => {
			...
			ch = sr.GetChecksum ();
	#if ASTARDEBUG
			Debuger.Log ("Got a whole bunch of data, "+bytes.Length+" bytes");
	#endif
			return true;
		}));
	...
	void DeserializeGraphs (byte[] bytes) {
		...
		AstarPath.active.AddWorkItem (new AstarPath.AstarWorkItem (force => {
			...
			if (sr.OpenDeserialize(bytes)) {
				...
				Debuger.LogWarning ("Invalid data file (cannot read zip).\nThe data is either corrupt or it was saved using a 3.0.x or earlier version of the system");
				// Make sure every graph has a graph editor
				CheckGraphEditors ();
			}
			return true;
