...
namespace Pathfinding {
	...
	public class EditorGUILayoutx {
		...
		public FadeArea BeginFadeArea (bool open, string id, float minHeight, GUIStyle areaStyle) {
			...
			if (editor == null) {
				Debuger.LogError ("You need to set the 'EditorGUIx.editor' variable before calling this function");
				return null;
			}
		...
		public void EndFadeArea () {
			...
			if (fadeAreaStack.Count <= 0) {
				Debuger.LogError ("You are popping more Fade Areas than you are pushing, make sure they are balanced");
				return;
			}
