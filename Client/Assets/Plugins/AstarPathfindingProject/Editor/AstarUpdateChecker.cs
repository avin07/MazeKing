...
namespace Pathfinding {
	...
	public static class AstarUpdateChecker {
		...
		public static System.DateTime lastUpdateCheck {
			get {
				try {
					...
					Debuger.LogWarning ("Invalid DateTime string encountered when loading from preferences");
				}
		...
		static bool CheckForUpdates () {
			...
			if (updateCheckDownload != null && updateCheckDownload.isDone) {
				...
				if (!string.IsNullOrEmpty (updateCheckDownload.error)) {
					Debuger.LogWarning ("There was an error checking for updates to the A* Pathfinding Project\n" +
		...
		static void ParseServerMessage (string result) {
			...
			#if ASTARDEBUG
			Debuger.Log ("Result from update check:\n"+result);
			#endif
			...
			try {
				...
				Debuger.LogWarning ("Could not parse version\n"+ex);
			}
		...
		static void ShowUpdateWindowIfRelevant () {
			try {
				...
				Debuger.LogError ("Invalid AstarRemindUpdateVersion or AstarRemindUpdateDate");
			}
