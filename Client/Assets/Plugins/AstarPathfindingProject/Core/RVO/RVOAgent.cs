	...
	public class Agent : IAgent {
		...
		public struct VO {
			...
			public VO ( Vector2 offset, Vector2 p0, Vector2 dir, float weightFactor ) {
				...
				//Debuger.DrawRay ( To3D(offset + line1), To3D(dir1)*10, Color.red);
			}
			...
			public VO ( Vector2 offset, Vector2 p1, Vector2 p2, Vector2 tang1, Vector2 tang2, float weightFactor ) {
				...
				//Debuger.DrawRay (To3D(cutoffLine+offset), To3D(cutoffDir)*10, Color.blue);
				//Debuger.DrawRay (To3D(line1+offset), To3D(dir1)*10, Color.blue);
				//Debuger.DrawRay (To3D(line2+offset), To3D(dir2)*10, Color.cyan);
			}
			...
			public VO ( Vector2 center, Vector2 offset, float radius, Vector2 sideChooser, float inverseDt, float weightFactor ) {
				...
				if ( center.magnitude < radius ) {
					...
					//Debuger.DrawRay ( To3D(line1), To3D(dir1), Color.cyan );
					//Debuger.DrawRay ( To3D(line2), To3D(dir2), Color.cyan );
				}
			}
		...
		internal void CalculateVelocity ( Pathfinding.RVO.Simulator.WorkerContext context ) {
			...
			if ( simulator.algorithm == Simulator.SamplingAlgorithm.GradientDecent ) {
				...
				/*for ( int i = 0; i < 3; i++ ) {
					...
					if ( score < best ) {
						//if ( score < best*0.9f ) Debuger.Log ("Better " + score + " < " + best);
						result = res;
						best = score;
					}
				}*/
