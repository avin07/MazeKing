...
#region Fields
		...
		public void LoadFromCache () {
			...
			if (file_cachedStartup != null) {
				...
				Debuger.LogError ("Can't load from cache since the cache is empty");
			}
		}
			...
			try {
				if (bytes != null) {
					...
					if (sr.OpenDeserialize(bytes)) {
						...
						Debuger.Log ("Invalid data file (cannot read zip).\nThe data is either corrupt or it was saved using a 3.0.x or earlier version of the system");
					}
				} else {
				...
				Debuger.LogWarning ("Caught exception while deserializing data.\n"+e);
				data_backup = bytes;
			}
			...
			for (int i=0;i<graphs.Length;i++) {
				for (int j=i+1;j<graphs.Length;j++) {
					if (graphs[i] != null && graphs[j] != null && graphs[i].guid == graphs[j].guid) {
						Debuger.LogWarning ("Guid Conflict when importing graphs additively. Imported graph will get a new Guid.\nThis message is (relatively) harmless.");
...
#if ASTARDEBUG
			Debuger.Log ("Found "+graphTypes.Length+" graph types");
#endif	
#else		
...
#region GraphCreation
		...
		public NavGraph CreateGraph (string type) {
			Debuger.Log ("Creating Graph of type '"+type+"'");
			...
			Debuger.LogError ("Graph type ("+type+") wasn't found");
			return null;
		}
		...
		public NavGraph AddGraph (string type) {
			...
			if (graph == null) {
				Debuger.LogError ("No NavGraph of type '"+type+"' could be found");
				return null;
			}
		...
		public NavGraph AddGraph (System.Type type) {
			...
			if (graph == null) {
				Debuger.LogError ("No NavGraph of type '"+type+"' could be found, "+graphTypes.Length+" graph types are avaliable");
				return null;
			}
...
#region GraphUtility
		...
		public int GetGraphIndex (NavGraph graph) {
			...
			Debuger.LogError ("Graph doesn't exist");
			return -1;
		}
