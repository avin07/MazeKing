...
#line hidden
	...
	private static void AstarLog (string s) {
		if (System.Object.ReferenceEquals(active,null)) {
			Debuger.Log ("No AstarPath object was found : "+s);
			return;
		}
		...
		if (threadEnumerator != null) {
			try {
				...
				if (!(e is ThreadControlQueue.QueueTerminationException)) {
					...
					Debuger.LogError ("Unhandled exception during pathfinding. Terminating.");
	...
	private int ProcessWorkItems (bool force) {
		if (pathQueue.AllReceiversBlocked) {
			...
			while (workItems.Count > 0) {
				...
				if (!status) {
					if ( force ) {
						Debuger.LogError ("Misbehaving WorkItem. 'force'=true but the work item did not complete.\nIf force=true is passed to a WorkItem it should always return true.");
					}
	...
	*/
				...
				if (s.order == GraphUpdateOrder.FloodFill) {
					...
					if (threading == GraphUpdateThreading.SeparateAndUnityInit) {
						try {
							...
							Debuger.LogError ("Error while initializing GraphUpdates\n" + e);
						}
					}
					try {
						s.graph.UpdateArea (s.obj);
					} catch (System.Exception e) {
						Debuger.LogError ("Error while updating graphs\n"+e);
					}
				}
#if !UNITY_WEBGL
			}
	...
	void ProcessGraphUpdatesAsync (System.Object _astar) {
		...
		if (System.Object.ReferenceEquals (astar, null)) {
			Debuger.LogError ("ProcessGraphUpdatesAsync started with invalid parameter _astar (was no AstarPath object)");
			return;
		}
		...
		while (!astar.pathQueue.IsTerminating) {
			...
			while (graphUpdateQueueAsync.Count > 0) {
				...
				try {
					...
					Debuger.LogError ("Exception while updating graphs:\n"+e);
				}
			}
	...
	public static int CalculateThreadCount (ThreadCount count) {
		...
		if (count == ThreadCount.AutomaticLowLoad || count == ThreadCount.AutomaticHighLoad) {
#if ASTARDEBUG
			Debuger.Log (SystemInfo.systemMemorySize + " " + SystemInfo.processorCount + " " + SystemInfo.processorType);
#endif
			...
			if ( memory <= 0 ) {
				Debuger.LogError ("Machine reporting that is has <= 0 bytes of RAM. This is definitely not true, assuming 1 GiB");
				memory = 1024;
			}
	...
	void Awake () {
		...
		if (FindObjectsOfType (typeof(AstarPath)).Length > 1) {
			Debuger.LogError ("You should NOT have more than one AstarPath component in the scene at any time.\n" +
		...
		for (int i=0;i<threads.Length;i++) {
			if (logPathResults == PathLog.Heavy)
				Debuger.Log ("Starting pathfinding thread "+i);
	...
	void OnDestroy () {
		...
		if (logPathResults == PathLog.Heavy)
			Debuger.Log ("+++ AstarPath Component Destroyed - Cleaning Up Pathfinding Data +++");
		...
		if (logPathResults == PathLog.Heavy)
			Debuger.Log ("Processing Eventual Work Items");
		...
		if (threads != null) {
			for (int i=0;i<threads.Length;i++) {
				...
				if (!threads[i].Join(200)) {
					Debuger.LogError ("Could not terminate pathfinding thread["+i+"] in 200ms." +
						"Not good.\nUnity webplayer does not support Thread.Abort\nHoping that it will be terminated by Unity WebPlayer");
				}
#else
				if (!threads[i].Join (50)) {
					Debuger.LogError ("Could not terminate pathfinding thread["+i+"] in 50ms, trying Thread.Abort");
					threads[i].Abort ();
				}
#endif
			}
		...
		if (logPathResults == PathLog.Heavy)
			Debuger.Log ("Cleaning up variables");
...
#region ScanMethods
	...
	public void FloodFill (GraphNode seed, uint area) {
		...
		if (area > GraphNode.MaxAreaIndex) {
			Debuger.LogError ("Too high area index - The maximum area index is " + GraphNode.MaxAreaIndex);
			return;
		}

		if (area < 0) {
			Debuger.LogError ("Too low area index - The minimum area index is 0");
			return;
		}
	...
	public void FloodFill () {
		...
		if (warnAboutAreas) {
			Debuger.LogError ("Too many areas - The maximum number of areas is " + GraphNode.MaxAreaIndex +". Try raising the A* Inspector -> Settings -> Min Area Size value. Enable the optimization ASTAR_MORE_AREAS under the Optimizations tab.");
		}
...
#if ASTARDEBUG
		Debuger.Log ("Flood fill complete, "+area+" area"+(area > 1 ? "s" : "")+" found - "+((System.DateTime.UtcNow.Ticks-startTime.Ticks)*0.0001).ToString ("0.00")+" ms");
#endif
	}
...
Debuger.Log ("Scanning... " + progress.description + " - " + (progress.progress*100).ToString ("0") + "%");
} \endcode
	  * \see Scan
	  */
	...
	public static void WaitForPath (Path p) {
		...
		if (waitForPathDepth == 5) {
			Debuger.LogError ("You are calling the WaitForPath function recursively (maybe from a path callback). Please don't do this.");
		}
...
\endcode
	...
	public static void StartPath (Path p, bool pushToFront = false) {
		...
		if (System.Object.ReferenceEquals (active, null)) {
			Debuger.LogError ("There is no AstarPath object in the scene");
			return;
		}
		...
		if (active.graphs == null || active.graphs.Length == 0) {
			Debuger.LogError ("There are no graphs in the scene");
			p.Error ();
			p.LogError ("There are no graphs in the scene");
			Debuger.LogError (p.errorLog);
			return;
		}
	...
	public void StartCoroutine (IEnumerator update) {
		if (activeThread != null && activeThread.IsAlive) {
			Debuger.Log ("Can only start one coroutine at a time, please end the current running thread first (activeThread)");
			return;
		}
	...
	public void OnApplicationQuit () {
		if (logPathResults == PathLog.Heavy) {
			Debuger.Log ("+++ Application Quitting - Cleaning Up +++");
		}
...
#region MainThreads
	...
	private static void CalculatePathsThreaded (System.Object _threadInfo) {
		...
		try {
			...
			Debuger.LogError ("Arguments to pathfinding threads must be of type ThreadStartInfo\n"+e);
		...
		try {
...
#endif
			{
				if (astar.logPathResults == PathLog.Heavy)
					Debuger.LogWarning ("Shutting down pathfinding thread #"+threadInfo.threadIndex);
				return;
			}
			Debug.LogException (e);
			Debuger.LogError ("Unhandled exception during pathfinding. Terminating.");
		...
		Debuger.LogError ("Error : This part should never be reached.");
