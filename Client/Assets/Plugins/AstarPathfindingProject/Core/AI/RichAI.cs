...
namespace Pathfinding {
	...
	public class RichAI : MonoBehaviour {
			...
			if (rp != null) {
				...
				if (fn != null) {
					...
					if (tg != lastTargetPoint) {
						...
						lastTargetPoint = tg;
						//Debuger.DrawRay (tr.position, Vector3.down*2,Color.blue,0.2f);
					}
					...
					if (wallForce > 0 && wallDist > 0) {
						...
						//Debuger.DrawRay (tr.position, force, Color.cyan);
					}
					...
					//Debuger.DrawRay (tr.position+Vector3.up, dir*3, Color.blue);
				...
				//w.Stop();
				//Debuger.Log ((w.Elapsed.TotalMilliseconds*1000));
		...
		Vector3 RaycastPosition (Vector3 position, float lasty) {
			if (raycastingForGroundPlacement) {
				...
				if (Physics.Raycast (position+Vector3.up*up,Vector3.down,out hit,up,groundMask)) {
					//Debuger.DrawRay (tr.position+Vector3.up*centerOffset,Vector3.down*centerOffset, Color.red);
					if (hit.distance < up) {
						//grounded
					...
					//Debuger.DrawRay (tr.position+Vector3.up*centerOffset,Vector3.down*centerOffset, Color.green);
				}
			}
			return position;
		}
		...
		IEnumerator TraverseSpecial (RichSpecial rs) {
			...
			if (al == null) {
				Debuger.LogError ("Unhandled RichSpecial");
				yield break;
			}
