...
public class AIPath : MonoBehaviour {
	 ...
	 * Note that this doesn't only affect the end point of the path
 	 * but also any intermediate points, so be sure to set #forwardLook and #pickNextWaypointDist to a higher value than this
 	 */
        public float slowdownDistance = 0.2f;//0.5f;
	
	/** Determines within what range it will switch to target the next waypoint in the path */
        public float pickNextWaypointDist = 1f;//1;
	...
	public virtual void SearchPath () {
        ...
        public virtual void SearchPath(Vector3 targetPosition)
        {
                lastRepath = Time.time;
                canSearchAgain = false;
                seeker.StartPath(GetFeetPosition(), targetPosition);
	...
	protected Vector3 CalculateVelocity (Vector3 currentPosition) {
		...
		Debuger.DrawRay (targetPosition,Vector3.up, Color.red);
		Debuger.DrawRay (GetFeetPosition(),dir,Color.yellow);
		Debuger.DrawRay (GetFeetPosition(),forward*sp,Color.cyan);
#endif
