...
namespace Pathfinding {
	...
	public abstract class Path {
		...
		~Path() {
			Debuger.Log ("Destroying " + GetType().Name + " instance");
			if (claimed.Count > 0) {
				Debuger.LogWarning ("Pool Is Leaking. See list of claims:\n" +
				...
				for (int i=0;i<claimed.Count;i++) {
					Debuger.LogWarning ("- Claim "+ (i+1) + " is by a " + claimed[i].GetType().Name + "\n"+claimInfo[i]);
				}
			} else {
				Debuger.Log ("Some scripts are not using pooling.\n" + pathTraceInfo + "\n");
			}
		}
#endif
...
\endcode
		 ...
		 * Debuger.Log call is only made if AstarPath.logPathResults is not equal to None and not equal to InGame.
...
\endcode
		...
		protected virtual void Trace (PathNode from) {
			...
			while (c != null) {
				...
				count++;
				if (count > 1024) {
					Debuger.LogWarning ("Inifinity loop? >1024 node path. Remove this message if you really have that long paths (Path.cs, Trace function)");
					break;
				}
			}
