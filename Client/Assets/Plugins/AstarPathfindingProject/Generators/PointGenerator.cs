...
\shadowimage{pointgraph_inspector.png}
	...
	,IUpdatableGraph
	{
		...
		public override NNInfo GetNearestForce (Vector3 position, NNConstraint constraint) {
			//Debuger.LogError ("This function (GetNearest) is not implemented in the navigation graph generator : Type "+this.GetType ().Name);
			...
			if ( optimizeForSparseGraph ) {
				...
				for ( int w = 1; w <= mw; w++ ) {
					...
					if ( w >= 20 ) {
						Debuger.LogWarning ("Aborting GetNearest call at maximum distance because it has iterated too many times.\n" +
