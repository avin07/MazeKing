		...
		static GridNode JPSJumpStraight ( GridNode node, Path path, PathHandler handler, int parentDir, int depth=0) {
			...
			while(true) {
				...
				if ( handler.GetPathNode(node).flag2 ) {
					//Debuger.Log ("Found end Node!");
					//Debuger.DrawRay ((Vector3)position, Vector3.up*2, Color.green);
					result = node;
					break;
				}
			...
			for (int i=0;i<8;i++) {
				if ( ((nb >> i)&1) != 0 ) {
					...
					if ( other != null ) {
						...
						//Debuger.DrawRay ( (Vector3)other.position, Vector3.up, Color.cyan);
						//GridNode other = nodes[nodeInGridIndex + neighbourOffsets[i]];
						...
						if (otherPN.pathID != pid) {
							...
							//Debuger.Log ("G " + otherPN.G + " F " + otherPN.F);
							handler.PushNode (otherPN);
							//Debuger.DrawRay ((Vector3)otherPN.node.Position, Vector3.up,Color.blue);
						} else {
				...
				if (i == 0 && parentDir != -1 && this.nodeInGridIndex > 10) {
					...
					if ( nodeInGridIndex + neighbourOffsets[oi] < 0 || nodeInGridIndex + neighbourOffsets[oi] >= nodes.Length ) {
						//Debuger.LogError ("ERR: " + (nodeInGridIndex + neighbourOffsets[oi]) + " " + cyclicParentDir + " " + parentDir + " Reverted " + oi);
						//Debuger.DrawRay ((Vector3)position, Vector3.up, Color.red);
					} else {
						GridNode other = nodes[nodeInGridIndex + neighbourOffsets[oi]];
