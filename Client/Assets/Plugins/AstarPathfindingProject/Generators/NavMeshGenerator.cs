	...
	,IRaycastableGraph 
	{
		...
		public static NNInfo GetNearest (NavMeshGraph graph, GraphNode[] nodes, Vector3 position, NNConstraint constraint, bool accurateNearestNode) {
			if (nodes == null || nodes.Length == 0) {
				Debuger.LogError ("NavGraph hasn't been generated yet or does not contain any nodes");
				return new NNInfo ();
			}
		...
		public override NNInfo GetNearestForce (Vector3 position, NNConstraint constraint) {
			...
			//Debuger.LogWarning ("This function shouldn't be called since constrained nodes are sent back in the GetNearest call");
		...
		public static void BuildFunnelCorridor (INavmesh graph, List<GraphNode> path, int startIndex, int endIndex, List<Vector3> left, List<Vector3> right) {
			...
			if (graph == null) {
				Debuger.LogError ("Couldn't cast graph to the appropriate type (graph isn't a Navmesh type graph, it doesn't implement the INavmesh interface)");
				return;
			}
		...
		public static bool Linecast (INavmesh graph, Vector3 tmp_origin, Vector3 tmp_end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
			...
			if (node == null) {
				...
				if (node == null) {
					Debuger.LogError ("Could not find a valid node to start from");
			...
			while (true) {
				...
				counter++;
				if ( counter > 2000 ) {
					Debuger.LogError ("Linecast was stuck in infinite loop. Breaking.");
				...
				if (newNode == null) {
					...
					//Ok, this is wrong...
					Debuger.LogWarning ("Linecast failing because point not inside node, and line does not hit any edges of it");
		...
		public void ScanInternal (string objMeshPath) {
			...
			if (mesh == null) {
				Debuger.LogError ("Couldn't read .obj file at '"+objMeshPath+"'");
				return;
			}
		...
		void GenerateNodes (Vector3[] vectorVertices, int[] triangles, out Vector3[] originalVertices, out Int3[] vertices) {
			...
			for (int i=0;i<nodes.Length;i++) {
				...
				if (a1 * a2 < 0) Debuger.LogError (a1+ " " + a2);
...
#endif
			//Debuger.Log ("Graph Generation - NavMesh - Time to compute graph "+((Time.realtimeSinceStartup-startTime)*1000F).ToString ("0")+"ms");
		}
