	...
	,IRaycastableGraph
	{
...
#if ASTARDEBUG
			Debuger.Log ("+++ --- GridGraph Setting Up Offsets and Costs");
#endif
		...
		public override void ScanInternal (OnScanStatus statusCallback) {
			...
			if (this.useJumpPointSearch) {
				Debuger.LogError ("Trying to use Jump Point Search, but support for it is not enabled. Please enable it in the inspector (Grid Graph settings).");
			}
#endif
		...
		public virtual void ErodeWalkableArea (int xmin, int zmin, int xmax, int zmax) {
			...
			if (!erosionUseTags) {
				...
				if (erodeIterations+erosionFirstTag > 31) {
					Debuger.LogError ("Too few tags available for "+erodeIterations+" erode iterations and starting with tag " + erosionFirstTag + " (erodeIterations+erosionFirstTag > 31)");
					return;
				}
				if (erosionFirstTag <= 0) {
					Debuger.LogError ("First erosion tag must be greater or equal to 1");
					return;
				}
		...
		public override void PostDeserialization () {
			...
			if (width*depth != nodes.Length) {
				Debuger.LogError ("Node data did not match with bounds data. Probably a change to the bounds/width/depth data was made after scanning the graph just prior to saving it. Nodes will be discarded");
			...
			for (int z = 0; z < depth; z ++) {
				for (int x = 0; x < width; x++) {
					...
					if (node == null) {
						Debuger.LogError ("Deserialization Error : Couldn't cast the node to the appropriate type - GridGenerator");
						return;
					}
