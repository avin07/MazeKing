	...
	public class LayerGridGraph : GridGraph, IUpdatableGraph {
		...
		public new void UpdateArea (GraphUpdateObject o) {
			...
			if (nodes == null || nodes.Length != width*depth*layerCount) {
				Debuger.LogWarning ("The Grid Graph is not scanned, cannot update area ");
				//Not scanned
				return;
			}
			...
			if (o.trackChangedNodes	&& willChangeNodeInstances) {
				Debuger.LogError ("Cannot track changed nodes when creating or deleting nodes.\nWill not update LayerGridGraph");
				return;
			}
...
#if ASTARDEBUG
							if (!node.Walkable)
								Debuger.DrawRay ((Vector3)node.position, Vector3.up*2,Color.red);
#endif
							CalculateConnections (nodes,node,x,z,y);
						}
					}
		...
		public override void ScanInternal (OnScanStatus statusCallback) {
			...
			if (width > 1024 || depth > 1024) {
				Debuger.LogError ("One of the grid's sides is longer than 1024 nodes");
				return;
			}
			...
			if (layerCount > LevelGridNode.MaxLayerCount) {
				Debuger.LogError ("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (found "+layerCount+")");
				return;
			}
				...
				do {
					...
					if (layerIndex >= layerCount) {
						if (layerIndex+1 > LevelGridNode.MaxLayerCount) {
							Debuger.LogError ("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (required "+(layerIndex+1)+")");
							return addedNodes;
						}
		...
		public void AddLayers (int count) {
			...
			if (newLayerCount > LevelGridNode.MaxLayerCount) {
				Debuger.LogError ("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (required "+newLayerCount+")");
				return;
			}
		...
		public override void ErodeWalkableArea (int xmin, int zmin, int xmax, int zmax) {
			...
			if (erosionUseTags) {
				Debuger.LogError ("Erosion Uses Tags is not supported for LayerGridGraphs yet");
			}
		...
		public override NNInfo GetNearest (Vector3 position, NNConstraint constraint, GraphNode hint) {
			...
			if (nodes == null || depth*width*layerCount != nodes.Length) {
				//Debuger.LogError ("NavGraph hasn't been generated yet");
				return new NNInfo ();
			}
			...
			for (int w = 1;;w++) {
				...
				for (nx = x-w;nx <= x+w;nx++) {
					...
					if (node != null) {
						...
						//Debuger.DrawRay (nodes[nx+nz*width].position,Vector3.up*dist,Color.cyan);counter++;
...
#if ASTARDEBUG
			Debuger.Log ("Grid Graph - Post Deserialize");
#endif
				...
				if (conn != LevelGridNode.NoConnection) {
					...
					if (otherPN.pathID != handler.PathID) {
						...
						if (pathNode.G+tmpCost+path.GetTraversalCost(other) < otherPN.G) {
							//Debuger.Log ("Path better from " + NodeIndex + " to " + otherPN.node.NodeIndex + " " + (pathNode.G+tmpCost+path.GetTraversalCost(other)) + " < " + otherPN.G);
						...
						//Or if the path from this node ("other") to the current ("current") is better
							...
							//Debuger.Log ("Path better from " + otherPN.node.NodeIndex + " to " + NodeIndex + " " + (otherPN.G+tmpCost+path.GetTraversalCost (this)) + " < " + pathNode.G);
