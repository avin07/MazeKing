	...
	#region Constants /** @name Constants @{ */
			...
			for (int z=0, pz = 0;z < wd;z += voxelArea.width, pz++) {
				for (int x=0;x < voxelArea.width;x++) {
					...
					for (int s = z+x; s != -1 && spans[s].bottom != VoxelArea.InvalidSpanValue; s = spans[s].next) {
						...
						//Debuger.DrawRay (p,voxelArea.VectorDirection[d]*cellSize*0.5F,Color.red);
...
#else
			Debuger.LogError ("This debug method only works with !ASTAR_RECAST_CLASS_BASED_LINKED_LIST");
#endif
		}
		...
		public void BuildCompactField () {
			...
			if (this.voxelWalkableHeight >= 0xFFFF) {
				Debuger.LogWarning ("Too high walkable height to guarantee correctness. Increase voxel height or lower walkable height.");
			}
							...
							for (int k=(int)nc.index, nk = (int)(nc.index+nc.count); k<nk; k++) {
								...
								if ((top-bottom) >= voxelWalkableHeight && System.Math.Abs ((int)ns.y - (int)s.y) <= voxelWalkableClimb) {
									...
									if (connIdx > MaxLayers) {
										Debuger.LogError ("Too many layers");
										continue;
									}
		...
		public void ErodeVoxels (int radius) {
			...
			if (radius > 255) {
				Debuger.LogError ("Max Erode Radius is 255");
				radius = 255;
			}
