	...
	public partial class Voxelize {
		...
		public ushort[] ExpandRegions (int maxIterations, uint level, ushort[] srcReg, ushort[] srcDist, ushort[] dstReg, ushort[] dstDist, List<int> stack) {
				...
				for (int x=0;x < voxelArea.width;x++) {
					...
					for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; i++) {
						...
						if (voxelArea.dist[i] >= level && srcReg[i] == 0 && voxelArea.areaTypes[i] != UnwalkableArea) {
							...
							st2.Add (z);
							st2.Add (i);
							//Debuger.DrawRay (ConvertPosition(x,z,i),Vector3.up*0.5F,Color.cyan);
						}
					}
				}
			}
			...
			if (stCount > 0) while (true) {
				...
				for (int j=0;j<stCount;j += 3) {
					...
					if (i < 0) {
						//Debuger.DrawRay (ConvertPosition(x,z,i),Vector3.up*2,Color.blue);
						failed++;
						continue;
					}
					...
					if (r != 0) {
						...
						failed++;
						//Debuger.DrawRay (ConvertPosition(x,z,i),Vector3.up*2,Color.red);
					}
				...
				if (failed*3 >= stCount) {
					//Debuger.Log("Failed count broke "+failed);
					break;
				}
		...
		public bool FloodRegion (int x, int z, int i, uint level, ushort r, ushort[] srcReg, ushort[] srcDist, List<int> stack) {
			...
			while (stack.Count > 0) {
				...
				//Debuger.DrawRay (ConvertPosition(cx,cz,ci),Vector3.up, Color.cyan);
		...
		public void BuildRegions () {
				...
				for (int j=0;j<st1.Count;j++) {
					...
					for (int dir = 0; dir < 4; dir++) {
						...
						if (srcReg[ni] == 0) {
							...
							if (!same) {
								srcReg[ni] = r;
								//Debuger.DrawRay (ConvertPosition(x,z,i),Vector3.up,Mathfx.IntToColor((int)level,0.6f));
			...
			for (int z=0, pz = 0;z < wd;z += voxelArea.width, pz++) {
				for (int x=0;x < voxelArea.width;x++) {
					...
					CompactVoxelCell c = voxelArea.compactCells[x+z];
					//if (c.count == 0) {
					//	Debuger.DrawRay (p,Vector3.up,Color.red);
					//}
					...
					for (int i = (int)c.index; i < c.index+c.count; i++) {
						...
						//Debuger.DrawRay (p,Vector3.up*0.5F,Color.green);
					}
				}
			}
