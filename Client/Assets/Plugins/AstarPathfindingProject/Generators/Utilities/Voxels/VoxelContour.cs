		...
		public void BuildContours (float maxError, int maxEdgeLength, VoxelContourSet cset, int buildFlags) {
			...
			for (int i = 0; i < contours.Count; i++)
			{
				...
				if (CalcAreaOfPolygon2D(cont.verts, cont.nverts) < 0)
				{
					...
					if (mergeIdx == -1)
					{
						Debuger.LogError ("rcBuildContours: Could not find merge target for bad contour "+i+".");
					}
					else
					{
						// Debugging
						//Debuger.LogWarning ("Fixing contour");
						...
						if (ia == -1 || ib == -1)
						{
							Debuger.LogWarning ("rcBuildContours: Failed to find merge points for "+i+" and "+mergeIdx+".");
							continue;
						}
						...
						{
							Debuger.LogWarning ("rcBuildContours: Failed to merge contours "+i+" and "+mergeIdx+".");
							continue;
						}
			...
			while (iter++ < 40000) {
				...
				if ((flags[i] & (ushort)(1 << dir)) != 0) {
					...
					*/
					...
					//Debuger.DrawRay (previousPos,new Vector3 ((dir == 1 || dir == 2) ? 1 : 0, 0, (dir == 0 || dir == 1) ? 1 : 0),Color.cyan);
					...
					if (ni == -1)
					{
						Debuger.LogWarning ("Degenerate triangles might have been generated.\n" +
