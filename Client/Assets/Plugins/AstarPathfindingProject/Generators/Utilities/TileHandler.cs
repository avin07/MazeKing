...
namespace Pathfinding.Util {
	public class TileHandler {
		...
		void CutPoly (Int3[] verts, int[] tris, ref Int3[] outVertsArr, ref int[] outTrisArr, out int outVCount, out int outTCount, Int3[] extraShape, Int3 cuttingOffset, Bounds realBounds, CutMode mode = CutMode.CutAll | CutMode.CutDual, int perturbate = 0) {
			...
			if (perturbate > 10) {
				Debuger.LogError ("Too many perturbations aborting : " + mode);
				Debug.Break();
			...
			for (int i=0;i<navmeshCuts.Count;i++) {
				...
				if (IntRect.Intersects(b3,bounds)) {
					...
					for (int j=origCutCount;j<cutVertices.Count;j++) {
						...
						if (cut.Count == 0) {
							Debuger.LogError ("Zero Length Contour");
			...
			while ( true ) {
				...
				for (int i=0;i<cutVertices.Count;i++) {
					...
					if (IntRect.Intersects(triBounds, cutBounds[i]) && !(ymax < tpYMin || ymin > tpYMax) && (cutsAddedGeom[i] || addIndex == -1) ) {
						...
						Debuger.DrawRay ((Vector3)tp1, Vector3.right*0.2f, Color.green);
						Debuger.DrawRay ((Vector3)tp1, Vector3.left*0.2f, Color.green);
						Debuger.DrawRay ((Vector3)tp1, Vector3.forward*0.2f, Color.green);
						Debuger.DrawRay ((Vector3)tp1, Vector3.back*0.2f, Color.green);
					...
					for (int exp=0;exp<sol.ChildCount;exp++) {
						...
						if (holes.Count == 0 && outer.Count == 3 && addIndex == -1) {
							for (int i=0;i<outer.Count;i++) {
								...
								if (det == 0) {
									Debuger.LogWarning ("Degenerate triangle");
									continue;
								}
							...
							while (contour != null) {
								...
								for (int i=0;i<contour.Count;i++) {
									...
									Debuger.DrawRay(new Vector3(contour[i].X,offset,contour[i].Y)*Int3.PrecisionFactor, Vector3.up, Color.cyan);
		#endif
							...
							try {
								...
								Debuger.LogWarning ("PointOnEdgeException, perturbating vertices slightly ( at "+cmode+" in " + mode +")");
