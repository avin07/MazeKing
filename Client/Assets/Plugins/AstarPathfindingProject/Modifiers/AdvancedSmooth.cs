	...
	public class AdvancedSmooth : MonoModifier {
		...
		public override void Apply (Path p, ModifierData source) {
			...
			for (int i=1;i<vectorPath.Length-1;i++) {
				...
				if (i == vectorPath.Length-2) {
					...
					Debuger.Log ("=== Right: "+betaRight+" Left: "+betaLeft+" ===");
				...
				/*Debuger.Log ("Gamma: "+ToDegrees(gamma1));
				...
				Debuger.Log ("Adding Circle Segment "+ToDegrees(gamma).ToString ()+" "+ToDegrees (va).ToString ()+" "+ToDegrees (delta).ToString ());
		...
		public class MaxTurn : TurnConstructor {
			...
			public override void OnTangentUpdate () {
				//Debuger.Log ("Updating Changed Tangent");
			...
			public override void PointToTangent (List<Turn> turnList) {
				...
				//Debuger.Log ("=== "+delta+" "+gammaRight+" Turning Rad "+turningRadius);
				//Debug.DrawLine (rightCircleCenter+ AngleToVector (gammaRight)*turningRadius, rightCircleCenter, Color.green);
		...
		public abstract class TurnConstructor {
			...
			public void AddCircleSegment (double startAngle, double endAngle, bool clockwise, Vector3 center, List<Vector3> output, float radius) {
				...
				//Debuger.Log ("Added Circle Segment Start: "+ToDegrees(startAngle) +" End: "+ToDegrees (endAngle)+" Clockwise: "+clockwise+" Step: "+step
		...
		public struct Turn : IComparable<Turn> {
			...
			public int CompareTo (Turn t) {
				//Debuger.Log ("Comparing "+t.id+" with "+id+" - scores "+t.score+" and "+score);
