	...
	public class SimpleSmoothModifier : MonoModifier {
		...
		public override void Apply (Path p, ModifierData source) {
			...
			if (p.vectorPath == null) {
				Debuger.LogWarning ("Can't process NULL path (has another modifier logged an error?)");
				return;
			}
		...
		public List<Vector3> CurvedNonuniform (List<Vector3> path) {
			...
			if (maxSegmentLength <= 0) {
				Debuger.LogWarning ("Max Segment Length is <= 0 which would cause DivByZero-exception or other nasty errors (avoid this)");
				return path;
			}
		...
		public List<Vector3> SmoothOffsetSimple (List<Vector3> path) {
			...
			if (iterations > 12) {
				Debuger.LogWarning ("A very high iteration count was passed, won't let this one through");
				return path;
			}
			...
			if (uniformLength) {
				...
				for (int i=0;i<path.Count-1;i++) {
					...
					for (int q=0;q<numSegmentsForSegment;q++) {
						//Debuger.Log (q+" "+c+" "+numSegments+" "+length+" "+numSegmentsForSegment);
