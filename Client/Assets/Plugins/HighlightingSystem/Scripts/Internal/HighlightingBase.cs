	...
	public class HighlightingBase : MonoBehaviour
	{
		...
		public void RenderHighlighting(RenderTexture frameBuffer)
		{
			...
			if (!isSupported || !enabled || !go.activeInHierarchy) { return; }
                        ...
                        int aa = 2;// QualitySettings.antiAliasing;
			if (aa == 0) { aa = 1; }
			...
			// If frameBuffer.depthBuffer is not available
                        int w = (int)(Screen.width * 0.75f);
                        int h = (int)(Screen.height * 0.75f);
			...
			#endif
			shaderCamera.Render();
                        //shaderCamera.targetTexture = highlightingBuffer;
		...
		static public void RemoveHighlighter(Highlighter h)
		{
			...
			if (index != -1)
			{
                ...
                public void SetParameterForHome()
                {
                        downsampleFactor = 2;
                        iterations = 3;
                        blurMinSpread = 0.3f;
