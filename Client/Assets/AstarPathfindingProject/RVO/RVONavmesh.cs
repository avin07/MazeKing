	...
	public class RVONavmesh : GraphModifier {
		...
		public void AddGraphObstacles (Pathfinding.RVO.Simulator sim, NavGraph graph) {
			if (obstacles.Count > 0 && lastSim != null && lastSim != sim) {
				Debuger.LogError ("Simulator has changed but some old obstacles are still added for the previous simulator. Deleting previous obstacles.");
				RemoveObstacles ();
			}
				...
				if (node != null) {
					...
					for (int j=0;j<3;j++) {
						...
						if (uses[j] == 0) {
							...
							Debuger.DrawRay (v1,Vector3.up*wallHeight,Color.red);
		#endif
