...
*/
	public class RecastGraph : NavGraph, INavmesh, IRaycastableGraph, IFunnelGraph, IUpdatableGraph, INavmeshHolder {
		...
		public void GetRecastMeshObjs (Bounds bounds, List<ExtraMesh> buffer) {
			...
			for (int i=0;i<buffer2.Count;i++) {
				...
				if (filter != null && rend != null) {
					...
					if (coll == null) {
						Debuger.LogError ("RecastMeshObject ("+buffer2[i].gameObject.name +") didn't have a collider or MeshFilter+Renderer attached");
						continue;
					}
		...
		static void GetSceneMeshes (Bounds bounds, List<string> tagMask, LayerMask layerMask, List<ExtraMesh> meshes) {
			...
			if ( (tagMask != null && tagMask.Count > 0) || layerMask != 0 ) {
				...
				for (int i=0;i<filteredFilters.Count;i++) {
					...
					if (containedStatic)
						Debuger.LogWarning ("Some meshes were statically batched. These meshes can not be used for navmesh calculation" +
		...
		public override NNInfo GetNearestForce (Vector3 position, NNConstraint constraint) {
			...
			//Debuger.DrawRay (new Vector3(tx,0,tz)*cellSize*tileSize + forcedBounds.min, Vector3.up*15, Color.blue);
			//Debuger.DrawRay (new Vector3(tx,0,tz)*cellSize*tileSize + forcedBounds.min + Vector3.forward*5, Vector3.back*10, Color.blue);
			//Debuger.DrawRay (new Vector3(tx,0,tz)*cellSize*tileSize + forcedBounds.min + Vector3.left*5, Vector3.right*10, Color.blue);
		...
		protected void BuildTileMesh (Voxelize vox, int x, int z) {
			...
			for (int i=0;i<mesh.verts.Length;i++) {
				...
				// Debug code
				//Debuger.DrawRay (matrix.MultiplyPoint3x4(vertices[i]),Vector3.up,Color.red);
			}
		...
		void ConnectTiles (NavmeshTile tile1, NavmeshTile tile2) {
			...
			if (Math.Abs (t1coord-t2coord) != 1) {
				...
				Debuger.Log (tile1.x + " " + tile1.z + " " + tile1.w + " " + tile1.d + "\n"+
		...
		public void ReplaceTile (int x, int z, int w, int d, Int3[] verts, int[] tris, bool worldSpace) {
			...
			if (!worldSpace) {
				if (!Mathf.Approximately (x*tileSizeX*cellSize*Int3.FloatPrecision, (float)Math.Round(x*tileSizeX*cellSize*Int3.FloatPrecision))) Debuger.LogWarning ("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
				if (!Mathf.Approximately (z*tileSizeZ*cellSize*Int3.FloatPrecision, (float)Math.Round(z*tileSizeZ*cellSize*Int3.FloatPrecision))) Debuger.LogWarning ("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
		...
		void CollectTreeMeshes (Terrain terrain, List<ExtraMesh> extraMeshes) {
			...
			for (int i=0;i<data.treeInstances.Length;i++) {
				...
				if (collider == null) {
...
#if ASTARDEBUG
					Debuger.DrawRay (instance.position, Vector3.up, Color.red,1);
#endif
					extraMeshes.Add (m);
				} else {
					...
					if (m.vertices != null) {
#if ASTARDEBUG
						Debuger.DrawRay (pos, Vector3.up, Color.yellow,1);
#endif
		...
		bool CollectMeshes (out List<ExtraMesh> extraMeshes, Bounds bounds) {
			...
			if (extraMeshes.Count == 0) {
				Debuger.LogWarning ("No MeshFilters where found contained in the layers specified by the 'mask' variables");
		...
		ExtraMesh RasterizeCollider (Collider col, Matrix4x4 localToWorldMatrix) {
			if (col is BoxCollider) {
				...
				for (int i=0;i<tris.Length;i+=3) {
					...
					Debuger.DrawRay ((va+vb+vc)/3, Vector3.Cross(vb-va,vc-va).normalized,Color.blue);*/
				}
#endif
				return m;
				...
				if (rows > 100) {
					Debuger.LogWarning ("Very large detail for some collider meshes. Consider decreasing Collider Rasterize Detail (RecastGraph)");
				}
