...
public class RaidHero : PropertyBase
{
        ...
        public RaidHero(SCMsgRaidHero msg)
        {
                m_ID = msg.idHero;
                PetID = msg.idPet;
                m_CharacterId = msg.idCharactar;
                m_nLevel = msg.nLevel;

                SetProperty("hp", msg.lHp);
                SetProperty("max_hp", msg.lMaxHp);
                SetProperty("pressure", msg.lPressure);
                SetProperty("max_pressure", msg.lMaxPressure);
                SetProperty("buff_list", msg.buffInfo);
                SetProperty("state", msg.nState);
        ...
        void OnBuffListUpdate(string name, string oldval, string newval)
        {
                ...
                foreach (string buff in buffs)
                {
                        ...
                        HeroBuff oldhb = oldlist.Find((x) => { return x.cfgId == hb.cfgId; });
