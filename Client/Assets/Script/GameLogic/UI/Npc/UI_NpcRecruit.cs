...
public class UI_NpcRecruit : UIBehaviour
{
        ...
        {
                anim.Stop();
                GameUtility.ObjPlayAnim(npcObj, "skill_001", false, 0.5f);
        }
        ...
        void Update()
        {              
                if (anim != null && anim.IsPlaying("skill_001"))
                {
                        if (anim["skill_001"].normalizedTime >= 0.9f)
                        {
                                GameUtility.ObjPlayAnim(npcObj, "idle_001", true);
                        }
                }
        }
