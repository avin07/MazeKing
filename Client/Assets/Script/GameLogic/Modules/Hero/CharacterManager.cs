...
class CharacterManager : SingletonObject<CharacterManager>
{
        ...
        public int GetCharacterModelId(int id)
        {
            if (m_CharacterDict.ContainsKey(id))
            {
                return m_CharacterDict[id].model_id;
            }
            return 0;
        }
        ...
        public string GetCharacterIcon(int id)
        {
                if (m_CharacterDict.ContainsKey(id))
                {
                        return ModelResourceManager.GetInst().GetIconRes(m_CharacterDict[id].model_id);
                }
                return string.Empty;
        }
        ...
        public GameObject GenerateModel(CharacterConfig cfg)
        {
                if (cfg != null)
                {
                        GameObject mod = ModelResourceManager.GetInst().GenerateObject(cfg.model_id);
                        if (mod != null)
                        {
