...
// 				range.xMax = Mathf.Max(range.yMax, m_AniCurve[n].time);
 	...
 	public override int GetAnimationState()
	{
		if (enabled == false || IsActive(gameObject) == false)
			return -1;
// 		Debuger.Log(m_fStartTime);
// 		Debuger.Log(IsEndAnimation());
...
// 		Debuger.Log("NcCurveAnimation.Awake " + transform.parent.name);
	}

	void Start()
	{
//		Debuger.Log("NcCurveAnimation.Start " + transform.parent.name);
	...
	void LateUpdate()
	{
// 		Debuger.Log("NcCurveAnimation.Update " + transform.parent.name);
		if (m_fStartTime == 0)
			return;
...
// 					{
// 						Debuger.Log(GetHelpString("DUPLICATE", string.Format("CurveIndex [{0}], TypeValue[{1}]", nIndex.ToString(), m_CurveInfoList[nIndex].GetValueName(n))));
// 						bDup = true;
// 					}
// 			}
// 		}
