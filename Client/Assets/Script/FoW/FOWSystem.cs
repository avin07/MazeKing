...
public class FOWSystem : MonoBehaviour
{
	...
	public Vector2 heightRange = new Vector2(4f, 10f);
	...
	public void Init ()
	{
        ...
        public void ResetWorldSize(int size)
        {
                worldSize = size;
                mSize = new Vector3(worldSize, heightRange.y - heightRange.x, worldSize);
                mOrigin = mTrans.position;
        ...
        public void ResetTextureSize(int size)
        {
                Color32 color = new Color32(255, 255, 0, 0);
                textureSize = size;
                mHeights = new int[textureSize, textureSize];
                int tex_size = textureSize * textureSize;
                mBuffer0 = new Color32[tex_size];
                mBuffer1 = new Color32[tex_size];
                mBuffer2 = new Color32[tex_size];
	...
	void ThreadUpdate ()
	{
		...
		for (; ; )
		{
			if (mState == State.NeedUpdate)
			{
				...
				UpdateBuffer();
				sw.Stop();
				if (debug) Debuger.Log(sw.ElapsedMilliseconds);
	...
	void RevealUsingRadius (Revealer r, float worldToTex)
	{
		...
		int ymin = Mathf.RoundToInt((pos.z - r.outerz) * worldToTex);
		int xmax = Mathf.RoundToInt((pos.x + r.outer) * worldToTex);
		int ymax = Mathf.RoundToInt((pos.z + r.outerz) * worldToTex);
		...
		//int radius = Mathf.RoundToInt(r.outer * r.outerz * worldToTex * worldToTex);
	...
	void UpdateTexture ()
	{
		...
		else if (mState == State.UpdateTexture1)
		{
			...
			mState = State.Blending;
                ...
                //GameObject.Find("testplane").GetComponent<MeshRenderer>().material.SetTexture("_MainTex", mTexture0);
//                  if (UIManager.GetInst().GetUIBehaviour<UI_RaidSkill>() != null)
//                          UIManager.GetInst().GetUIBehaviour<UI_RaidSkill>().PrepareMask(mTexture1);
	}
	...
	public bool IsExplored (Vector3 pos)
	{
        ...
        public void SetBufferUnexplored(int posx, int posz)
        {
                float worldToTex = (float)textureSize / worldSize;
                // Position relative to the fog of war
                Vector3 pos = new Vector3(posx, 0, posz) - mOrigin;
                // Coordinates we'll be dealing with
        ...
        public void RevealSinglePoint(int posx, int posz, bool bLog = false)
        {
                lock (mBuffer1)
                {
                        float worldToTex = (float)textureSize / worldSize;
