...
public class UI_PetMain : UI_ScrollRectHelp
{
        ...
        private void OnTab(GameObject go, PointerEventData data)
        {
                ...
                if (pt != m_tab)
                {
                        if (pt == TAB.Advanced)
                        {
                                if (target.Equals(CommonString.zeroStr) || target.Length == 0)
                                {
        ...
        public void RefreshLeft()
        {
                ...
                ResourceManager.GetInst().LoadIconSpriteSyn(CharacterManager.GetInst().GetCareerIcon(m_pet.GetPropertyInt("career")), GetImage(left, "career").transform);
        ...
        public void OnLevel() //升级
        {
                ...
                if (PetManager.GetInst().ChoosePetList.Count > 0)
                {
                        ...
                        {
                                food += PetManager.GetInst().ChoosePetList[i] + CommonString.pipeStr;
                        }
                }
        ...
        void RefreshAdvancedTarget()
        {
                ...
                GameObject one = GetGameObject(advanced_pet_group, CommonString.zeroStr);
                GameObject two = GetGameObject(advanced_pet_group, CommonString.oneStr);
        ...
        void RefreshPet(Pet pet, GameObject go)
        {
                ...
                ResourceManager.GetInst().LoadIconSpriteSyn(CharacterManager.GetInst().GetCareerIcon(pet.GetPropertyInt("career")), GetImage(go, "career").transform);
        ...
        void OnTarget(GameObject go, PointerEventData data)
        {
                ...
                if (target_num == 2) //置灰另外一个
                {
                        for (int i = 0; i < target_num; i++)
                        {
                                GameObject pet_obj = GetGameObject(GetGameObject(advanced_pet_group, CommonString.oneStr), "pet" + i);
        ...
        void RefreshAttrGroup(Pet target_pet)
        {
                ...
                else
                {
                        ...
                        GetText(attrs_obj[0], "name0").text = LanguageManager.GetText(CharacterManager.GetInst().GetCareerDic(old_career).name);
                        GetText(attrs_obj[0], "name1").text = LanguageManager.GetText(CharacterManager.GetInst().GetCareerDic(target_career).name);
                }
                ...
                int index = 3;
                string[] skill_list_old = (m_pet.GetPropertyString("skill_list") + CommonString.commaStr + m_pet.GetPropertyString("passive_skill")).Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                string[] skill_list_new = (target_pet.GetPropertyString("skill_list") + CommonString.commaStr + target_pet.GetPropertyString("passive_skill")).Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        void RefreshNeedItem(string item_need)  //物品
        {
                ...
                if (item_need.Equals(CommonString.zeroStr))  //不需要物品
                {
