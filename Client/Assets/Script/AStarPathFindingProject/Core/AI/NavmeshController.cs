...
public class NavmeshController : MonoBehaviour {
	...
	public Vector3 SimpleMove (Vector3 currentPosition, Vector3 direction) {
		...
		if (controller == null) {
			Debuger.LogError ("No CharacterController is attached to the GameObject");
			return direction;
		}
	...
	public GraphNode ClampAlongNavmesh (Vector3 startPos, GraphNode _startNode, Vector3 endPos, out Vector3 clampedPos) {
		...
		if (graph == null) {
			//Debuger.LogError ("Null graph, or the graph was no NavMeshGraph");
			return startNode;
		}
		...
		while (stack.Count > 0)
		{
			...
			{
#if ASTARDEBUG
				Debuger.DrawRay (endPos,Vector3.up,Color.red);
#endif
				bestRef = cur;
				bestPos = endPos;
				break;
