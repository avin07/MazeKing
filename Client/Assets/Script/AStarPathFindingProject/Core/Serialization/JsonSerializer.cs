	...
	public class GraphSerializationContext {
		...
		public void SerializeUnityObject ( UnityEngine.Object ob ) {
			...
			if (component != null || go != null) {
				...
				if (helper == null) {
					Debuger.Log ("Adding UnityReferenceHelper to Unity Reference '"+ob.name+"'");
		...
		public UnityEngine.Object DeserializeUnityObject ( ) {
			...
			if (type == null) {
				Debuger.LogError ("Could not find type '"+typename+"'. Cannot deserialize Unity reference");
				return null;
			}
		...
		public void SerializeExtraInfo () {
			...
			for (int i=0;i<graphs.Length;i++) {
				...
				graphs[i].GetNodes (node => {
					...
					if (node.NodeIndex == -1) {
						Debuger.LogError ("Graph contains destroyed nodes. This is a bug.");
					}
					return true;
				});
			}
...
#region Deserialize
		...
		public bool OpenDeserialize (byte[] bytes) {
			...
			try {
				...
				Debuger.LogWarning ("Caught exception when loading from zip\n"+e);
			...
			if (FullyDefinedVersion(meta.version) > FullyDefinedVersion(AstarPath.Version)) {
				Debuger.LogWarning ("Trying to load data from a newer version of the A* Pathfinding Project\nCurrent version: "+AstarPath.Version+" Data version: "+meta.version +
				...
				Debuger.LogWarning ("Trying to load data from an older version of the A* Pathfinding Project\nCurrent version: "+AstarPath.Version+" Data version: "+meta.version+
