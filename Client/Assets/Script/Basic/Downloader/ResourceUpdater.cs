...
public class ResourceUpdater : SingletonObject<ResourceUpdater>
{
        ...
        public static readonly string LOCAL_ADD_RES_URL = Application.persistentDataPath + CommonString.divideStr;
        ...
        void DownloadStreaming()
        {
                if (NeedDownFiles.Count <= 0)
                {
                        ...
                        {
                                versions.Append(item.Key).Append(CommonString.commaStr).Append(item.Value).Append("\n");
                        }
...
#endif
                        localFile = localFile.Replace("\\", CommonString.divideStr);
        ...
        private void DownLoadRes()
        {
                ...
                update_file_url = update_file_url.Replace("\\", CommonString.divideStr);
        ...
        private void UpdateLocalRes(string fileName, byte[] data)  //把需要下载的新增到可读写的地址//
        {
                ...
                fileName = fileName.Replace("\\", CommonString.divideStr);
                string filePath = LOCAL_ADD_RES_URL + fileName;
                filePath = filePath.Replace("\\", CommonString.divideStr);
