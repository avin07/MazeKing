						...
						while (oleDbDataReader.Read())
						{
							if (!ExcelFile.IsHeadRow(oleDbDataReader, layout))
							{
								...
								for (int i = 0; i < oleDbDataReader.FieldCount; i++)
								{
									if (layout.typed)
									{
										stringBuilder.AppendFormat("\t\t<{0} type='{1}'>{2}</{0}>\n", layout[i].name, ExcelXMLLayout.Type2Int(layout[i].type), ((DbDataReader)oleDbDataReader)[i].ToString());
									}
									else
									{
										stringBuilder.AppendFormat("\t\t<{0}>{1}</{0}>\n", layout[i].name, ((DbDataReader)oleDbDataReader)[i].ToString());
									}
								}
								stringBuilder.AppendFormat("\t</{0}>\n", layout.name);
							}
